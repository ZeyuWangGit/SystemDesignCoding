...
public static class Carpool
{
    ...
    */
    ...
    {
        var graph = new Dictionary<string, List<(string destination, int duration)>>();
        foreach (var (origin, destination, duration) in roads)
        {
            if (!graph.ContainsKey(origin))
            {
                graph.Add(origin, [(destination, duration)]);
            }
            else
            {
                graph[origin].Add((destination, duration));
            }
        }

        var cars = starts.Select(start => new CarState
        {
            CurrentLocation = start,
            Passengers = [],
            TimeElapsed = 0,
        }).ToList();
        var locationToPeopleDict = new Dictionary<string, List<string>>();
        foreach (var (name, location) in people)
        {
            if (!locationToPeopleDict.ContainsKey(location))
            {
                locationToPeopleDict.Add(location, []);
            }
            locationToPeopleDict[location].Add(name);
        }

        while (cars.Any(car => car.CurrentLocation != "Campground"))
        {
            foreach (var car in cars)
            {
                if (car.CurrentLocation == "Campground")
                {
                    continue;
                }
                var nextStops = graph[car.CurrentLocation];
                var (destination, duration) = nextStops.MinBy(stop => stop.duration);
                if (locationToPeopleDict.ContainsKey(car.CurrentLocation))
                {
                    car.Passengers.AddRange(locationToPeopleDict[car.CurrentLocation]);
                    locationToPeopleDict.Remove(car.CurrentLocation);
                }
                car.CurrentLocation = destination;
                car.TimeElapsed += duration;
                
            }
        }

        return cars.Select(car => car.Passengers).ToList();
    }
