...
public static class ThrillingTeleporters
{
    /*
...
*/
    public static HashSet<int> Destinations(string[] teleporters, int dieSides, int start, int end)
    {
        ...
        foreach (var teleporter in teleporters)
        {
            ...
            teleporterMap.Add(from, to);
        }

        var res = new HashSet<int>();

        for (var i = 1; i <= dieSides; i++)
        {
            var curr = start + i;
            if (curr > end)
            {
            ...
            res.Add(teleporterMap.GetValueOrDefault(curr, curr));
        }
        return res;
    }
}