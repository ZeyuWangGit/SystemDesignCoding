...
public class TokenBucketRateLimiterTests
{
    ...
    {
        var rateLimiter = new TokenBucketRateLimiter(5, 2); // 每个用户最大 5 个令牌，1 秒补充 2 个
        int customerId = 1;
    ...
    {
        var rateLimiter = new TokenBucketRateLimiter(2, 1); // 2 个令牌，1 秒补充 1 个
        int customerId = 1;
    ...
    {
        var rateLimiter = new TokenBucketRateLimiter(2, 2); // 1 秒补充 2 个令牌
        int customerId = 1;
        ...
        Thread.Sleep(2000); // 等待 2 秒，应该补充 4 个令牌
    ...
    {
        var rateLimiter = new TokenBucketRateLimiter(3, 2); // 最大 3 个令牌，1 秒补充 2 个
        int customerId = 1;
    ...
    {
        var rateLimiter = new TokenBucketRateLimiter(2, 1); // 每个用户 2 个令牌，1 秒补充 1 个
    ...
    {
        var rateLimiter = new TokenBucketRateLimiter(5, 1);
        int customerId = 1;
        int allowedCount = 0;
        int threadCount = 10;
